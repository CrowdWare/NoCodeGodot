...
if (( ${#POSITIONAL_ARGS[@]} > 1 )); then
  EXTRA_RUNNER_ARGS=("${POSITIONAL_ARGS[@]:1}")
fi
load_local_run_include
...
if [[ -z "$MODE" ]]; then
  ...
  echo "  5) none      -> ohne URL-Override (letzte URL beibehalten)"
...
case "$MODE" in
  default)
    ...
    exec "$GODOT_BIN" "${GODOT_ARGS[@]-}" --path "$RUNNER_PATH" -- --url "$DEFAULT_UI" "${EXTRA_RUNNER_ARGS[@]-}" "${FORGE_RUNNER_ARGS[@]-}"
    ;;
  designer|sample)
    require_godot
    echo "Starting ForgeRunner with docs/ForgeDesigner/app.sml"
    exec "$GODOT_BIN" "${GODOT_ARGS[@]-}" --path "$RUNNER_PATH" -- --url "$SAMPLE_UI" "${EXTRA_RUNNER_ARGS[@]-}" "${FORGE_RUNNER_ARGS[@]-}"
    ;;
  docking)
    require_godot
    echo "Starting ForgeRunner with samples/docking_demo.sml"
    exec "$GODOT_BIN" "${GODOT_ARGS[@]-}" --path "$RUNNER_PATH" -- --url "$DOCKING_SAMPLE_UI" "${EXTRA_RUNNER_ARGS[@]-}" "${FORGE_RUNNER_ARGS[@]-}"
    ;;
  poser)
    require_godot
    echo "Starting ForgePoser..."
    exec "$GODOT_BIN" "${GODOT_ARGS[@]-}" --path "$RUNNER_PATH" -- --url "$POSER_UI" "${EXTRA_RUNNER_ARGS[@]-}" "${FORGE_RUNNER_ARGS[@]-}"
    ;;
  none)
    require_godot
    echo "Starting ForgeRunner ohne URL-Override (letzte URL bleibt aktiv)"
    exec "$GODOT_BIN" "${GODOT_ARGS[@]-}" --path "$RUNNER_PATH" -- "${EXTRA_RUNNER_ARGS[@]-}" "${FORGE_RUNNER_ARGS[@]-}"
    ;;
  docs)
    require_godot
