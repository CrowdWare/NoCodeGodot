...
func _get_layout_defaults_auto_fill_types() -> Array[String]:
    if not SPECS.has("layout_defaults"):
        return []

    var values := Array(SPECS["layout_defaults"].get("autoFillMaxSizeTypes", []))
    var out: Array[String] = []
    for value in values:
        var type_name := String(value)
        if type_name == "":
            continue
...
func _generate_schema_layout_defaults_cs() -> void:
    ...
    for type_name in _get_layout_defaults_auto_fill_types():
        cs += "        \"%s\",\n" % _cs_string(type_name)

    cs += "    ];\n\n"
    cs += "    private static readonly ISet<string> AutoFillMaxSizeTypeSet = BuildAutoFillMaxSizeTypeSet();\n\n"
    cs += "    private static ISet<string> BuildAutoFillMaxSizeTypeSet()\n"
    cs += "    {\n"
    cs += "        return new HashSet<string>(AutoFillMaxSizeTypes, StringComparer.OrdinalIgnoreCase);\n"
    cs += "    }\n\n"
