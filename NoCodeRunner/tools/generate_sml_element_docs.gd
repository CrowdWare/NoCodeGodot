...
const OUT_DIR := "res://docs/SML/Elements"
const EXTRA_CLASSES := ["Window", "PopupMenu"]  # CHANGED
const REF_PATH := "res://docs/sms-reference.sml"  # CHANGED
...
# CHANGED: Heuristic detection of "collection" controls that manage internal items (not child nodes).
func _is_collection_control(c_name: String) -> bool:
    return _has_count_property(c_name) or _has_item_methods(c_name) or _has_item_signals(c_name)
