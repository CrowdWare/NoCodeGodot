# AbstractPolygon2DEditor

## Inheritance

[AbstractPolygon2DEditor](AbstractPolygon2DEditor.md) → [HBoxContainer](HBoxContainer.md) → [BoxContainer](BoxContainer.md) → [Container](Container.md) → [Control](Control.md) → [CanvasItem](CanvasItem.md) → [Node](Node.md) → [Object](Object.md)

## Collection Items

This control appears to manage internal **items** (collection-style API).
Items are typically not represented as child nodes/properties in Godot.
In SML, this will be represented via **pseudo child elements** (documented per control).

## Derived Classes

### Direct subclasses

- [CollisionPolygon2DEditor](CollisionPolygon2DEditor.md)
- [LightOccluder2DEditor](LightOccluder2DEditor.md)
- [Line2DEditor](Line2DEditor.md)
- [NavigationObstacle2DEditor](NavigationObstacle2DEditor.md)
- [NavigationRegion2DEditor](NavigationRegion2DEditor.md)
- [Polygon2DEditor](Polygon2DEditor.md)

## Properties

This page lists **only properties declared by `AbstractPolygon2DEditor`**.
Inherited properties are documented in: [HBoxContainer](HBoxContainer.md)

| Godot Property | SML Property | Type | Default |
|-|-|-|-|

## Events

This page lists **only signals declared by `AbstractPolygon2DEditor`**.
Inherited signals are documented in: [HBoxContainer](HBoxContainer.md)

| Godot Signal | SMS Event | Params |
|-|-|-|

## SML Items (TODO)

This control appears to manage internal items, but a dedicated SML pseudo-child specification has not been defined yet.
Use the generated signals and the `collection: true` marker in `sms-reference.sml` as implementation hints.
