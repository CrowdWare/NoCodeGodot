...
- Provides C ABI entry points:
  ...
  - `sms_native_set_sandbox_path_callback(...)` (host-enforced FS sandbox policy hook)
...
- `os.fileExists(...)` and `fs.list/readText/writeText` support host-controlled policy:
  - with callback: host enforces trust mode/sandbox policy
  - without callback: trusted-local compatibility fallback
